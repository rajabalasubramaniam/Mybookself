...
function BookCard({ book }) {
    return (
        <Link href={`/books/${book.id}`} className="bg-white rounded-xl shadow hover:shadow-lg overflow-hidden">
            <div className="flex">
                {book.cover_url ? (
        ...
        <div className="min-h-screen bg-gray-50">
            <nav className="bg-white shadow-sm">
                <div className="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
                    <Link href="/dashboard" className="text-2xl font-bold text-blue-900">ReadWise</Link>
                    <Link href="/books/add" className="bg-blue-900 text-white px-4 py-2 rounded-lg">
                        + Add Book
                    </Link>
                </div>
            </nav>
